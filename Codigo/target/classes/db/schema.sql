...
CREATE TABLE pessoa (id INTEGER NOT NULL, nome VARCHAR(50) NOT NULL, email VARCHAR(50) NOT NULL, imagem VARCHAR(60),PRIMARY KEY(id));
...
-- CRIAR TABELA AULAO
CREATE TABLE aulao (local VARCHAR(100) NOT NULL, data_hora VARCHAR(30), materia INTEGER NOT NULL, nome VARCHAR(50), PRIMARY KEY(local, data_hora), FOREIGN KEY(materia) REFERENCES materia(cod_materia) ON DELETE SET NULL);

-- CRIAR TABELA REPOSITORIO
CREATE TABLE repositorio (nome VARCHAR(50) NOT NULL, cod_materia INTEGER NOT NULL, PRIMARY KEY(nome), FOREIGN KEY(cod_materia) REFERENCES materia(cod_materia) ON DELETE SET NULL);
...
CREATE TABLE atendimento (matricula_a INTEGER NOT NULL, matricula_m INTEGER NOT NULL, horario VARCHAR(10) NOT NULL, data VARCHAR(20) NOT NULL, cod_materia INTEGER NOT NULL, descricao VARCHAR(100) NULL, PRIMARY KEY(matricula_a, matricula_m, horario, data, cod_materia), FOREIGN KEY(matricula_a) REFERENCES aluno(matricula), FOREIGN KEY(matricula_m) REFERENCES monitor(matricula), FOREIGN KEY(cod_materia) REFERENCES monitor(cod_materia));
